...
Outliers detection provides a workflow to automatically detect and alert when the volume of events generated by a data source or data host goes beyond a usual volume.
...
- Should outliers be detected based on the policy, which is customisable om a per source basis, the data source or host will be reported in alert
